...
def test_harika_resume_parses_exactly_three_education_records():
    from sources.pdf_parser import parse_resume
    res = parse_resume("data/3_CHANDRA_HARIKA.pdf")
    assert len(res["education"]) == 3
    # Check details of SRM IST
    srm = next(e for e in res["education"] if "SRM" in e["institution"])
    assert srm["degree"] == "B.Tech"
    assert srm["end_year"] == 2027

def test_vishwas_email_is_correctly_extracted():
    from sources.pdf_parser import parse_resume
    res = parse_resume("data/2_Vishwas_Reddy.pdf")
    assert "vishwasr762@gmail.com" in res["emails"]

def test_projects_extracted_from_resume():
    from sources.pdf_parser import parse_resume
    
    # 1. Vishnupriya Anamalamudi
    res_v = parse_resume("data/vishnupriya_resume.pdf")
    assert len(res_v["projects"]) == 3
    
    p1 = next(p for p in res_v["projects"] if "Crop Stress" in p["name"])
    assert "Explainable Edge AI for Crop Stress Detection" in p1["name"]
    assert "Python" in p1["tech_stack"]
    assert "Grad-CAM" in p1["tech_stack"]
    assert p1["duration"] == "Ongoing"
    assert "tomato" in p1["description"]
    
    p2 = next(p for p in res_v["projects"] if "Symptom" in p["name"])
    assert "Symptom-Based Illness Prediction Chatbot" in p2["name"]
    assert "Python" in p2["tech_stack"]
    assert "Streamlit" in p2["tech_stack"]
    assert "RandomForest" in p2["description"]
    
    p3 = next(p for p in res_v["projects"] if "SplitEase" in p["name"])
    assert "SplitEase" in p3["name"]
    assert "React.js" in p3["tech_stack"]
    assert "MongoDB" in p3["tech_stack"]
    assert "JWT" in p3["tech_stack"]
    assert "UPI" in p3["description"]

    # 2. Tanishka Agarwal
    res_t = parse_resume("data/tanishka_resume.pdf")
    assert len(res_t["projects"]) == 3
    
    tp1 = next(p for p in res_t["projects"] if "Expense" in p["name"])
    assert "Personal Expense Tracker" in tp1["name"]
    assert "C++" in tp1["tech_stack"]
    assert tp1["duration"] == "2023"
    
    tp2 = next(p for p in res_t["projects"] if "Judging" in p["name"])
    assert "Project Submission and Judging Platform" in tp2["name"]
    assert "Node.js" in tp2["tech_stack"]
    assert tp2["duration"] == "2025"

def test_projects_do_not_affect_experience_or_education_parsing():
    from sources.pdf_parser import parse_resume
    
    # Run education check on Harika
    res_h = parse_resume("data/3_CHANDRA_HARIKA.pdf")
    assert len(res_h["education"]) == 3
    
    # Run email check on Vishwas
    res_w = parse_resume("data/2_Vishwas_Reddy.pdf")
    assert "vishwasr762@gmail.com" in res_w["emails"]
